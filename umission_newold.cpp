...
bool UMission::mission1(int & state)
{
  ...
  switch (state)
  {
    ...
    case 10: // first PART - wait for IR2 then go fwd and turn
      ...
      /*
      // SQUARE DRIVE
      snprintf(lines[0], MAX_LEN, "vel=0.0, topos=0.0 : time= 1");
      snprintf(lines[1], MAX_LEN, "vel=0.7");
      snprintf(lines[2], MAX_LEN, "head=0.0 : dist=1.2");
      snprintf(lines[3], MAX_LEN, "head=90.0 : dist=1.2");
      snprintf(lines[4], MAX_LEN, "head=180.0 : dist=1.2");
      snprintf(lines[5], MAX_LEN, "head=270.0 : dist=1.2");
      snprintf(lines[6], MAX_LEN, "head=0.0");
      snprintf(lines[7], MAX_LEN, "vel=0.0, topos=0.0: time=1");
      //////////////
      */

      // LINE FOLLOW
      /*snprintf(lines[0], MAX_LEN, "vel=0.0, topos=0.0 : time= 1");
      snprintf(lines[1], MAX_LEN, "vel=0.5,edgel=0, white=0 : dist=2");
      snprintf(lines[2], MAX_LEN, "event=1, vel=0");
      snprintf(lines[3], MAX_LEN, ": dist=1");*/

      snprintf(lines[0], MAX_LEN, "vel=0.2, acc=1, edgel=0, white=1 : dist=0.5");
      ...
      snprintf(lines[1], MAX_LEN, "vel=0.5, acc=1, edgel=0, white=1 : ir1 < 0.15");
      //drive 25 cm to steer clear of the gate
      snprintf(lines[2], MAX_LEN, "vel=0.5, acc=1, edgel=0, white=1 : dist=0.25");
      ...
      // send the 4 lines to the REGBOT
      //sendAndActivateSnippet(lines, 9);
