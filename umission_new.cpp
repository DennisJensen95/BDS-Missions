...
bool UMission::mission3(int &state)
{
  ...
  case 21:
  {
    ...
    snprintf(lines[line++], MAX_LEN, "topos=0, vel=0.1, acc=0.3, head=%.1f: turn=-40, time=2", heading-40.0);
    // create event 1
  ...
  case 30: // move to ball
  {
    ...
    float dx = 0.32; // distance to stop in front of marker
    float dy = 0.09; // distance to the left of marker
